...
SELECT C.CODFILIAL,
       ...
       C.CODIGO AS CODCRED, --USADO NA CONSTRUCAO DA VIEW_JC_PCPREST_BAIXA_CRED       M.CODBANCO,
...
SELECT C.CODFILIAL,
       ...
       --M.DTCOMPENSACAO,
       (CASE WHEN M.DTCOMPENSACAO IS NULL THEN C.DTLANC ELSE M.DTCOMPENSACAO END) DATA, --PARA NAO FICAR SEM DATA EM SITUAÇÕES AONDE NAO FOI CONCILIADO
       0 AS CODCRED, --USADO NA CONSTRUCAO DA VIEW_JC_PCPREST_BAIXA_CRED       M.CODBANCO,
       B.CODCONTABIL AS CODCONTABILBANCO,
  ...
  FROM PCCRECLI C
 INNER JOIN PCMOVCR M
    ON C.NUMTRANSBAIXA = M.NUMTRANS   AND C.CODMOVIMENTO = M.CODBANCO
  LEFT JOIN PCBANCO B
    ON M.CODBANCO = B.CODBANCO
  LEFT JOIN PCCLIENT T
    ON C.CODCLI = T.CODCLI
