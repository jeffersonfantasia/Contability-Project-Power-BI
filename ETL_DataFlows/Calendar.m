...
  
  #"Cálculo Mês Atual" = Table.AddColumn(#"Sufixo Adicionado2", "Cálculo Mês Atual", each 
      ( Date.Year( [Data Base] ) - Date.Year( DataAtual ) ) * 12 
      + Date.Month( [Data Base] ) 
      - Date.Month( DataAtual ), Int64.Type),
  
  #"Cálculo Trimestre Atual" = Table.AddColumn(#"Cálculo Mês Atual", "Cálculo Trimestre Atual", each /*Year Difference*/
...
  
  #"Cálculo Ano Atual" = Table.AddColumn(#"Cálculo Trimestre Atual", "Cálculo Ano Atual", each 
      Date.Year( [Data Base] ) 
      - Date.Year( DataAtual ), Int64.Type),
  
  #"Cálculo Semana Atual" = Table.AddColumn(#"Cálculo Ano Atual", "Cálculo Semana Atual", each 
      ( Date.Year( [Data Base] ) - Date.Year( DataAtual ) ) *53 
      + Date.WeekOfYear( [Data Base] ) 
      - Date.WeekOfYear( DataAtual ), Int64.Type),
  
  #"Marcação Data Futura" = Table.AddColumn(#"Cálculo Semana Atual" , "Período", each 
      if [Data Base] = DataAtual 
      then "Hoje" 
      else if [Data Base] > DataAtual then "Futuro" 
      else "Passado", type text),
  
  #"Nº Mês-Ano" = Table.AddColumn(#"Marcação Data Futura", "Nº Mês-Ano", each 
      [Ano] * 100 + [Nº Mês], Int64.Type),
  
...
  
  #"Fim de Semana" = Table.AddColumn( #"Dia da Semana", "Semana ou Fim Semana", each 
