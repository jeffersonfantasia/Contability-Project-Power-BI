CREATE OR REPLACE VIEW VIEW_JC_LANC_CRED_AVULSO AS--LANCAMENTOS DE CREDITOS MOVIMENTANDO CONTA GERENCIAL--
WITH LANC_CRED_AVULSO AS
 (SELECT I.CODFILIAL,
   ...
   WHERE I.NUMLANC IS NOT NULL --APENAS LANÇAMENTOS QUE GERARAM PCLANC
     AND I.NUMERARIO = 'N' --APENAS LANÇAMENTOS QUE NÃO MOVIMENTARAM NUMERÁRIOS
     AND I.VALOR > 0 --APENAS REGISTROS QUE FORAM LANCAMENTOS DE CREDITO COM CONTA GERENCIAL
     AND NVL(I.CODMOVIMENTO, 0) NOT IN (37, 620108, 620109) --RETIRAR LANÇAMENTOS DE AJUSTE E TAXAS DE MKT 
  ),
LANC_NUMCRED AS
...
SELECT T.CODFILIAL,
 ...
 WHERE T.NUMLANCBAIXA IS NOT NULL --APENAS LANÇAMENTOS QUE GERARAM RECEITA NA PCLANC
   AND NVL(ORIGEM, 0) NOT IN ('M') --RETIRAR LANÇAMENTOS DE DIF ENTRE NOTA E FINANCEIRO
UNION ALL
...
SELECT T.CODFILIAL,
 ...
 INNER JOIN LANC_CRED_AVULSO F
  ...
  LEFT JOIN PCCLIENT C
    ON T.CODCLI = C.CODCLI
 WHERE T.NUMTRANSVENDADESC IS NOT NULL --APENAS LANÇAMENTOS QUE APLICADOS COMO DESCONTO DUPLICATA  
UNION ALL
...
UNION ALL
--BAIXA DOS CREDITOS AVULSO COMO MOVIMENTACAO DE NUMERÁRIO - BANCOS-- 
SELECT T.CODFILIAL,
       T.NUMTRANSBAIXA AS CODLANC,
       T.NUMCRED,
       T.CODIGO,
